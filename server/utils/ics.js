...
export function buildMeetingICS(m) {
  ...
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//ChatKonect//Meetings//EN',
    'CALSCALE:GREGORIAN',
    'METHOD:REQUEST',
    'BEGIN:VEVENT',
    `UID:${m._id}@chatkonect`,
    `DTSTAMP:${toICSDate(new Date())}`,
    `DTSTART:${toICSDate(start)}`,
    `DTEND:${toICSDate(end)}`,
    `SUMMARY:${esc(m.title || 'ChatKonect Meeting')}`,
    `DESCRIPTION:${esc(desc)}`,
    m.link ? `URL:${esc(m.link)}` : '',
    m.link ? `LOCATION:${esc(m.link)}` : '',
    m.hostName ? `ORGANIZER;CN=${esc(m.hostName)}:mailto:no-reply@chatkonect.app` : '',
    'STATUS:CONFIRMED',
    `SEQUENCE:0${rrule}`,
    'END:VEVENT',
    'END:VCALENDAR',
